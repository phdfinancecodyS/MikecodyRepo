...
_MAX_TOKENS = int(os.environ.get("CCE_LLM_RESPONDER_MAX_TOKENS", "80"))
_TEMPERATURE = float(os.environ.get("CCE_LLM_RESPONDER_TEMPERATURE", "0.6"))
_GUIDANCE_MODE = os.environ.get("CCE_LLM_GUIDANCE_MODE", "compact").strip().lower()
...
You are Ask Anyway's guide. Write one short response (max 40 words).
...
- Reflect with paraphrase first. Do not mirror user wording line-for-line.
...
TURN CONTEXT: This is turn 2. The user was asked "How are you doing?" and gave \
a negative or neutral response. Use MI: reflect what they shared using their \
words, then ask ONE open-ended question that invites them deeper.

User said: "{user_message}"

Generate one MI reflection + open question. Under 40 words."""

_DEEPENING_TEMPLATE = """\
TURN CONTEXT: This is turn 3 (MI deepening). The person has shared what is \
going on. Your job: complex reflection that shows you heard the thread \
of the whole conversation, then ask what part has been hardest or what \
...
Generate one complex reflection + open question. Under 40 words."""

_GOAL_CLARIFY_TEMPLATE = """\
TURN CONTEXT: This is turn 4 (SFBT pivot). The person has shared their \
struggle and you have reflected it back. Now shift from understanding the \
problem to imagining the first step of change. Use ONE of these SFBT moves:
- Future question: "If things got even a little better with this, what would \
you notice first?"
- Scaling: "Where are you with this right now, and what would one step up look like?"
- Exception: "Has there been a moment recently where this felt even a little \
more manageable?"

Do NOT repeat MI deepening. This turn is forward-looking.
...
Generate one SFBT-style forward question. Under 40 words."""
...
def _build_history_text(history: List[dict]) -> Optional[str]:
    ...
    compact_lines = [ln[:220] for ln in lines[-4:]]
    return "\n".join(compact_lines)
