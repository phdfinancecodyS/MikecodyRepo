...
**Known TODOs before launch** (stubs marked in code):
1. `topic-match/route.ts` — replace stub `matchDomain()` with config-driven matching against `quiz/topic-matcher-flow.json`
2. `recommendation/route.ts` — resolve human-readable guide title from `quiz/base-guide-catalog.json`
3. `webhooks/stripe/route.ts` — wire real email delivery (SendGrid/ConvertKit) and SMS enrollment (Twilio)
4. `analytics/event/route.ts` — route non-CTA events to analytics store (Mixpanel, PostHog, etc.)
5. Frontend — wire app pages to call endpoints in sequence; handle critical fast-path safely
...
- [ ] Frontend quiz UI — needs implementation
- [ ] Stripe product setup — needs live price IDs
- [ ] Email/SMS provider wiring — SendGrid/ConvertKit + Twilio integration
- [ ] Topic matcher logic — stub needs config-driven implementation
- [ ] Analytics persistence — currently logging to stdout only
