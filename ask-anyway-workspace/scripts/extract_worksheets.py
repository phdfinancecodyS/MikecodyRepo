...
def extract_worksheet(content, ws_num, ws_name):
    """Extract a specific worksheet section from the guide content."""
    header = f"## Worksheet {ws_num}: {ws_name}"
...
def process_file(fpath):
    ...
    for ws_num, ws_name in [(1, "Pattern Finder"), (2, "Action Builder")]:
        body = extract_worksheet(content, ws_num, ws_name)
        if not body:
