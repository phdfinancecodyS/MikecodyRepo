# Headache Functional Plan

Status: draft_v1_complete
Guide ID: split-14
Guide type: split
Source: Ch17
Batch: batch-2
Priority: medium
Audience Bucket: Military / Veteran
Audience Tier: primary
Base Guide Path: content/topic-guides/splits/split-14-headache-functional-plan.md

## What This Helps With

When the headache won't quit and you still have to function — this isn't medical advice. It's a plan for getting through the day.

This version is written for military and veteran life, where mission, hypervigilance, identity, loss of tribe, and carrying the past all shape how a problem lands.

## What Is Happening

Military life trains your body and mind around vigilance, control, performance, and mission. Even after service, those patterns don't disappear just because the environment changed. A lot of the struggle isn't weakness. It's training, injury, loss, and identity still showing up in civilian life.

Body-based symptoms can feel scary and make decision-making harder in the moment.
A short protocol helps you move from alarm to control.
You're aiming for steady function, not instant perfection.

## What To Do Now

1. Ask whether this reaction belongs to your current environment or to training and survival patterns that are still firing after the mission changed.
2. Name the symptom pattern and rate intensity from 1-10.
3. Run one grounding or pacing protocol for 90 seconds.
4. Choose one practical adjustment for the next hour.

## What To Say

- Part of this is service-related, and I'm not going to shame myself for the fact that it still shows up.
- I need support that respects the way training and loss changed me.
- My system is spiking right now, and I'm using my reset plan.
- I need two minutes to stabilize and then I can continue.
- I'm not in danger right now; I'm in a surge and taking control steps.

## Common Mistakes To Avoid

- Expecting civilian routines, relationships, or workplaces to make sense without accounting for service, transition, or loss of tribe.
- Ignoring early symptoms until they become overwhelming.
- Making big decisions during peak body alarm.
- Skipping follow-up care when patterns repeat.

## 24-Hour Action Plan

- Right now: lower the lights, drink a full glass of water, and take any prescribed medication. Don't power through — manage.
- Before bed tonight: eat something and set a consistent sleep time.
- Tomorrow: if it's still there, track what's different today — hydration, sleep, stress, caffeine — and start building the pattern.
- Peer support step: lean on one person from your unit, your post, a vet peer support network, or your VA team today — calling for backup isn't a weakness in the field and it isn't one now.

## Worksheet 1: Pattern Finder

Goal: Spot the pattern — when it shows up, what triggers it, and what you notice first.

Prompts:
- When did "Headache Functional Plan" last show up for you? What was the situation?
- What did you notice first — in your body, your thoughts, or your behavior?
- What makes this pattern worse? What conditions or stressors fuel it?
- What have you tried before, and what actually helped (even a little)?
- Who in your life knows about this? Who doesn't — and why?

## Worksheet 2: Action Builder

Goal: Turn what you noticed into one action and one commitment.

Prompts:
- One specific action I'll take in the next 24 hours:
- The sentence I'll use when I notice this pattern starting:
- The person I'll reach out to this week:
- What I'll do differently next time instead of the old response:
- By tomorrow night, I'll know this worked if:

---

**A quick note:** This guide is educational — it's not therapy, it's not a diagnosis, and reading it doesn't create a clinical relationship between us. It's built to help you take the next step, not replace professional support. If you're in crisis, contact 988 (Suicide & Crisis Lifeline), text HOME to 741741 (Crisis Text Line), or call 911.

---

> **Disclaimer:** This content is for educational purposes only and is not a substitute for professional mental health treatment. If you or someone you know is in crisis, contact the **988 Suicide & Crisis Lifeline** by calling or texting **988**, or text **HOME** to **741741** to reach the Crisis Text Line.
